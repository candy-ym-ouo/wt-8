      ...
      <div class="detail-main">
        ...
        <div class="detail-section card">
          <h3 class="section-title">
            ...
            <span class="comment-count">({{ crowdfunding.commentCount || 0 }})</span>
          </h3>
              ...
              <div class="comment-main">
                ...
                <div class="comment-body">
                  <div class="comment-header">
                    ...
                    <span v-if="crowdfunding.creatorId === comment.user?.id" class="author-badge">作者</span>
                    ...
                    <div v-for="reply in comment.replies" :key="reply.id" class="reply-item">
                      ...
                      <div class="reply-body">
                        <div class="reply-header">
                          ...
                          <span v-if="crowdfunding.creatorId === reply.user?.id" class="author-badge">作者</span>
...
const loadComments = async (page = 1) => {
  ...
  try {
    const res = await api.get(`/crowdfundings/${route.params.id}/comments?page=${page}&limit=20`)
...
const submitComment = async () => {
  ...
  try {
    ...
    await api.post(`/crowdfundings/${route.params.id}/comments`, payload)
    commentContent.value = ''
    replyTarget.value = null
    showToast('评论成功', 'success')
    loadComments(1)
    if (crowdfunding.value) {
      crowdfunding.value.commentCount = (crowdfunding.value.commentCount || 0) + 1
    }
  } catch (e) {
...
const deleteComment = async (comment, parentCommentId) => {
  ...
  try {
    await api.delete(`/crowdfundings/${route.params.id}/comments/${comment.id}`)
    showToast('已删除', 'success')
    loadComments(commentPage.value)
    if (crowdfunding.value && crowdfunding.value.commentCount > 0) {
      crowdfunding.value.commentCount -= 1
    }
  } catch (e) {
...
const toggleLike = async (comment, parentCommentId) => {
  ...
  try {
    const res = await api.post(`/crowdfundings/${route.params.id}/comments/${comment.id}/like`)
